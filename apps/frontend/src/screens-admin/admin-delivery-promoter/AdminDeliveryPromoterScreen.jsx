...
function AdminDeliveryPromoterScreen() {
  ...
  /*
   * No mountedRef here, deliberately - the `active` flag inside each fetching
   * effect is the staleness guard. See AdminDeliveryOverviewScreen for why the
   * ref that used to sit here broke every screen in this section.
   */
  ...
  useEffect(() => {
    ...
    deliveryApi.promoter(promoterId, { from: range.from, to: range.to })
      ...
      .catch((err) => { if (active) { setError(err?.message || 'Failed to load promoter report.'); setStatus('error'); } });
    return () => { active = false; };
